...
int inet_pton(int af, const char *src, void *dst)
{
    ...
    //check welke soort ipadress probeerd te connecten ipv6 or ipv4.
...
int connection(int internet_socket)
{
    ...
    snprintf(CLI_buffer, sizeof(CLI_buffer), "curl http://ip-api.com/json/%s", client_ip);
    FILE *fp;
    ...
    // Parse JSON response
    char *country = strtok(IP_LOG_ITEM, ",");
    char *regionName = strtok(NULL, ",");
    char *city = strtok(NULL, ",");
    char *isp = strtok(NULL, ",");
    char *org = strtok(NULL, ",");

    // Write geolocation information to IPLOG.txt
    ...
    if(logp != NULL)
    {
        ...
        {
            fprintf(logp, "+---------------------------------------------------+\n");
            fprintf(logp, "IP Address: %s\nGeen geolocatie beschikbaar: LocalHost\n", client_ip);
        }
        fclose(logp);
    }
    else
    {
        printf("Error opening IPLOG.txt\n");
    }
